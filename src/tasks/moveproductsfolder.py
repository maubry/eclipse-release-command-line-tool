...
class MoveProductsFolder:
    ...
    def check(self):
        ...
        # check folder to move is a folder in source path
        ...
        if not releaseutils.contains_products(self._source_path, self._product_filenames): 
        ...
        self._folder_dest_path = os.path.join(self._folder_name)
        if os.path.exists(self._dest_path):
            raise Exception("a file or folder {0} already exists".format(self._dest_path))
         
        return ["Products will be moved from {0} to {1}".format(self._source_path,self._dest_path)]    
    ...
    def run(self):
        ...
        return ["Products moved.".format(self._source_path,self._dest_path)]   
